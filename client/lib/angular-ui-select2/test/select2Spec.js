...
describe('uiSelect2', function () {
  ...
  describe('with a <select> element', function () {
    ...
    it('should observe an option with ng-repeat for changes', function(){
      scope.entities = ['first', 'second', 'third'];
      scope.foo = 'fourth';
      var element = compile('<select ui-select2 ng-model="foo"><option ng-repeat="item in entities">{{item}}</option></select>');
      expect(element.select2('val')).toBe(null);
      scope.$apply('foo="fourth";entities=["fourth"]');
      $timeout.flush();
